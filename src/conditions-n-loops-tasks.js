...
function canQueenCaptureKing(queen, king) {
  ...
  return queen.x + queen.y === king.x + king.y;
}
...
function convertToRomanNumerals(num) {
  let result = '';
  const deci = Math.trunc(num / 10);
  const units = num % 10;
  for (let i = 0; i < deci; i += 1) {
    result += 'X';
  }
  switch (units) {
    case 1:
...
function rotateMatrix(matrix) {
  ...
  if (rows !== cols) {
    throw new Error('In-place rotation only works for square matrices.');
  }
  ...
  for (let i = 0; i < rows; i += 1) {
    for (let j = 0; j < Math.floor(cols / 2); j += 1) {
      [matrix1[i][j], matrix1[i][cols - j - 1]] = [
        matrix1[i][cols - j - 1],
        matrix1[i][j],
      ];
    }
...
function sortByAsc(arr) {
  ...
  for (let i = 1; i < arr.length; i += 1) {
    const current = arr[i];
    let j;
    for (j = i - 1; j >= 0 && arr[j] > current; j -= 1) {
      result[j + 1] = arr[j];
    }
    result[j + 1] = current;
  }
...
function shuffleChar(str, iterations) {
  ...
  let result = str;
  let optimizedIteration = iterations;
  let iterationShift = 1;

  for (let i = 0; i < optimizedIteration; i += 1) {
    let odd = '';
    let even = '';
    ...
    result = even + odd;
    if (result === str) {
      optimizedIteration = iterations % iterationShift;
      i = -1;
    }
    iterationShift += 1;
  }
